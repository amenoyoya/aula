...
namespace Aula {
    inline void open_encoding_library(sol::state &lua) {
        ...
        auto str = lua["string"].get_or_create<sol::table>();

        str.set_function("same", sol::overload(
            [](const std::string &s1, const std::string &s2, bool compareCase, u32 len) { return Encoding::isSameString(s1, s2, compareCase, len); },
            [](const std::string &s1, const std::string &s2, bool compareCase) { return Encoding::isSameString(s1, s2, compareCase); },
            [](const std::string &s1, const std::string &s2) { return Encoding::isSameString(s1, s2); }
        ));
    }
}
