...
function FuncBlockInfo.new( declFuncInfo, nsInfo, funcType, typeDataAccessor, funcScope, tokenList, declPos, orgPos )
   local obj = {}
   FuncBlockInfo.setmeta( obj )
   if obj.__init then
...
function TransUnit:setup( src )
   ...
   self.globalScope = Ast.ScopeWithRef.new(self.processInfo, self.globalScope, src.globalScope, Ast.ScopeKind.Other)
...
function TransUnit.new( moduleId, importModuleInfo, macroEval, enableMultiPhase, analyzeModule, mode, pos, targetLuaVer, ctrl_info, builtinFunc )
   local obj = {}
...
function TransUnit:pushExtModule( externalFlag, name, accessMode, pos, lazy, lang, requirePath )
   ...
   if not self.typeId2ClassMap[typeInfo:get_typeId()] then
      local namespace = Nodes.NamespaceInfo.new(modName, self:get_scope(), typeInfo)
...
function TransUnit:dumpSymbolType( accessMode, name, typeInfo )
   ...
   local writer = Writer.JSON.new(Aula.IO.Stdout)
...
function TransUnit:analyzeMatch( firstToken )
   ...
   if not self.moduleType:equals( self.processInfo, algeTypeInfo:getModule(  ) ) then
      if not _lune._Set_has(self.importModuleSet, algeTypeInfo:getModule(  ) ) then
...
function TransUnit:analyzeRefType( accessMode, allowDDD, parentPub )
   ...
   if token.txt == "..." then
      ...
      name = Nodes.ExpRefNode.create( self.nodeManager, token.pos, self.inTestBlock, self.macroCtrl:isInAnalyzeArgMode(  ), {dddSym:get_typeInfo()}, Ast.AccessSymbolInfo.new(self.processInfo, dddSym, _lune.newAlge( Ast.OverrideMut.None), true) )
   else
   ...
   do
      ...
      if aliasType ~= nil then
         ...
         if not _lune._Set_has(self.importModuleSet, aliasSrc:getModule(  ) ) and self.moduleType:get_parentInfo() ~= aliasSrc:getModule(  ):get_parentInfo() then
            ...
            self:addErrMess( symbolNode:get_pos(), string.format( "must import '%s' for this alias -- %s (%s,%s)", aliasSrc:getModule(  ):getFullName( self.typeNameCtrl, self:get_scope(), false ), symbolNode:getSymbolInfo(  )[1]:get_name(), self.moduleType:get_typeId().id, aliasSrc:getModule(  ):get_typeId().id) )
         end
...
function TransUnit:analyzeDeclForm( accessMode, firstToken )
   ...
   local declFuncInfo = Nodes.DeclFuncInfo.new(Nodes.FuncKind.Form, nil, nil, false, name, formSymbol or shadowing, argList, false, accessMode, asyncMode, nil, retTypeList, retNodeList, false, false, 0)
...
function TransUnit:analyzeClassBody( hasProto, classAccessMode, firstToken, mode, gluePrefix, classTypeInfo, typeDataAccessor, name, moduleLang, moduleName, lazyLoad, nextToken, inheritInfo )
   ...
   local function processInitBlock( token )
      ...
      local info = Nodes.DeclFuncInfo.new(Nodes.FuncKind.InitBlock, classTypeInfo, node, true, token, _lune.unwrap( (funcSym or shadowing )), {}, true, Ast.AccessMode.Pri, nil, block, {}, {}, false, false, nsInfo:get_stmtNum())
...
function TransUnit:analyzeDeclFunc( declFuncMode, asyncLocked, abstractFlag, overrideFlag, accessMode, staticFlag, classTypeInfo, firstToken, name )
   ...
   local function createDeclFuncInfo( funcKind )
      ...
      local declFuncInfo = Nodes.DeclFuncInfo.new(funcKind, classTypeInfo, classDeclNode, outsizeOfClass, name, funcSym, argList, orgStaticFlag, accessMode, asyncMode, body, retTypeInfoList, retTypeNodeList, _lune._Set_has(self.has__func__Symbol, typeInfo ), overrideFlag, stmtNum)
      ...
      if needFuncBlockInfo then
         ...
         local funcBlockInfo = FuncBlockInfo.new(declFuncInfo, nsInfo, typeInfo, typeDataAccessor, funcBodyScope, blockTokenList, pos, orgPos)
...
function TransUnit:accessSymbol( symbolInfo, canLeftExp )
   ...
   if symbolInfo:get_kind() == Ast.SymbolKind.Fun then
      ...
      if self:get_scope():isClosureAccess( self.moduleScope, symbolInfo ) then
         ...
         table.insert( self.closureFunList, ClosureFun.new(symbolInfo, self:get_scope()) )
      end
...
function TransUnit:analyzeDeclVar( mode, accessMode, firstToken )
   ...
   local node = Nodes.DeclVarNode.create( self.nodeManager, firstToken.pos, self.inTestBlock, self.macroCtrl:isInAnalyzeArgMode(  ), {Ast.builtinTypeNone}, mode, accessMode, false, varList, expList, symbolInfoList, typeInfoList, unwrapFlag, unwrapBlock, thenBlock, syncSymbolList, syncBlock )
...
function TransUnit:analyzeIfUnwrap( firstToken )
   ...
   return Nodes.IfUnwrapNode.create( self.nodeManager, firstToken.pos, self.inTestBlock, self.macroCtrl:isInAnalyzeArgMode(  ), {Ast.builtinTypeNone}, varList, expList, block, elseBlock )
end
...
function TransUnit:processFuncBlockInfo( funcBlockCtlIF, streamName )
   ...
   while true do
      ...
      local funcBodyScope = Ast.ScopeWithRef.new(self.processInfo, outerScope, funcBlockInfo:get_funcScope(), Ast.ScopeKind.Other)
...
function TransUnit:analyzeListConst( token, expectType )
   ...
   if token.txt == '[' then
      ...
      return Nodes.LiteralArrayNode.create( self.nodeManager, token.pos, self.inTestBlock, self.macroCtrl:isInAnalyzeArgMode(  ), typeInfoList, expList )
   end
...
function TransUnit:analyzeMapConst( token, expectType )
   ...
   return Nodes.LiteralMapNode.create( self.nodeManager, token.pos, self.inTestBlock, self.macroCtrl:isInAnalyzeArgMode(  ), {typeInfo}, map, pairList )
end
...
function TransUnit:processFunc( firstToken, nextToken, refFieldNode, funcExp, funcType, alt2typeMap, genericTypeList, genericsClass, argList )
   ...
   if argList ~= nil then
      do
         ...
         elseif _switchExp == self.builtinFunc.list_sort or _switchExp == self.builtinFunc.array_sort then
...
function TransUnit:analyzeExpCall( firstToken, funcExp, nextToken )
   ...
   if funcTypeInfo:equals( self.processInfo, self.builtinFunc.set_add ) then
      if argList ~= nil then
         if argList:get_expType():get_nilable() then
...
function TransUnit:checkComp( token, callback )
   ...
   if self.analyzeMode == AnalyzeMode.Complete and self:isTargetToken( token ) then
      ...
      if currentModule == target then
         local jsonWriter = Writer.JSON.new(Aula.IO.Stdout)
...
function TransUnit:analyzeNewAlge( firstToken, algeTypeInfo, prefix )
   ...
   do
      ...
      if valInfo ~= nil then
         ...
         return Nodes.NewAlgeValNode.create( self.nodeManager, firstToken.pos, self.inTestBlock, self.macroCtrl:isInAnalyzeArgMode(  ), {algeTypeInfo}, symbolToken, prefix, algeTypeInfo, Ast.AlgeValInfo.new("", {}, algeTypeInfo, _lune.unwrap( dummySymbol)), {} )
      end
   end
...
function TransUnit:analyzeExpSymbol( firstToken, symbolToken, mode, prefixExp, skipFlag, canLeftExp )
   ...
   elseif mode == ExpSymbolMode.Symbol then
      if self.macroCtrl:get_analyzeInfo():isAnalyzingSymArg(  ) then
         ...
         local typeInfo = symbolInfo:get_typeInfo()
         if symbolInfo:get_kind() == Ast.SymbolKind.Typ and typeInfo:get_kind() == Ast.TypeInfoKind.Alternate and self.analyzingStaticMethodArgsScope ~= nil and symbolInfo:get_scope() ~= self.analyzingStaticMethodArgsScope then
            
            self:addErrMess( firstToken.pos, string.format( "The static method can't use the type parameter. -- %s", typeInfo:getTxt(  )) )
         end
         ...
         exp = Nodes.ExpRefNode.create( self.nodeManager, firstToken.pos, self.inTestBlock, self.macroCtrl:isInAnalyzeArgMode(  ), {typeInfo}, Ast.AccessSymbolInfo.new(self.processInfo, symbolInfo, _lune.newAlge( Ast.OverrideMut.None), true) )
      end
...
function TransUnit:analyzeExp( allowNoneType, skipOp2Flag, canLeftExp, prevOpLevel, expectType )
   ...
   elseif token.kind == Parser.TokenKind.Symb and token.txt == "__line__" then
      ...
      exp = Nodes.LiteralIntNode.create( self.nodeManager, firstToken.pos, self.inTestBlock, self.macroCtrl:isInAnalyzeArgMode(  ), {Ast.builtinTypeInt}, Parser.Token.new(Parser.TokenKind.Int, string.format( "%d", pos.lineNo), token.pos, false, nil), token.pos.lineNo )
   ...
   elseif token.kind == Parser.TokenKind.Type then
      ...
      exp = Nodes.ExpRefNode.create( self.nodeManager, firstToken.pos, self.inTestBlock, self.macroCtrl:isInAnalyzeArgMode(  ), {Ast.builtinTypeNone}, Ast.AccessSymbolInfo.new(self.processInfo, symbolTypeInfo, _lune.newAlge( Ast.OverrideMut.None), false) )
...
function TransUnit:analyzeReturn( token )
   ...
   if nsInfo:isLockedAsync(  ) then
      self:addErrMess( token.pos, "can't use 'return' in the __asyncLock." )
   end
