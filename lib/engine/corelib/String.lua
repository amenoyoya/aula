...
function string:u8judge()
    local ret = Aula.Encoding.isUTF8(self)
...
function string:encode(toEnc, fromEnc)
    return Aula.Encoding.encode(self, toEnc, fromEnc or Aula.Encoding.UNKNOWN)
end

function string:u8encode(fromEnc)
    return Aula.Encoding.toUTF8(self, fromEnc or Aula.Encoding.UNKNOWN)
end

function string:encoding()
    return Aula.Encoding.getName(self)
end


-- バイナリ文字列に数値を入力・出力
function string:push(val, byte)
    byte = byte or 4
    for i = 0, byte - 1 do
        self = self..string.char(bit.band(bit.rshift(val, i * 8), 0xff))
    end
    return self
end

function string:pop(head, byte)
    local val = 0
    head = head or 1
    byte = byte or 4
    for i = 0, byte - 1 do
        val = bit.bor(val, (bit.lshift(self:byte(i + head), i * 8)))
    end
    return val
end
...
function println(...)
    ...
    local n = #list
    if n == 0 then
        Aula.Console.writeLine "nil"
    else
        Aula.Console.write(tostring(list[1]))
        for i = 2, n do
            Aula.Console.write("\t"..tostring(list[i]))
        end
        Aula.Console.write "\n"
    end
end
...
function printf(format, ...)
    Aula.Console.write(format:format(...))
end

-- コマンドラインから文字列を読み込む(UTF-8)
function Aula.Console.readLine(message)
    if type(message) == 'string' then
        Aula.Console.write(message)
    end
    return io.read():u8encode()
end

readln = Aula.Console.readLine
...
-- 大文字・小文字を無視して文字列比較
function string:same(str, len)
    return Aula.isSameString(self, str, len or -1)
end
