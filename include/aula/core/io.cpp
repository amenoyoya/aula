...
namespace aula {
    namespace io {
        ...
        bool binary_encrypt(binary_t *self, const std::string &key, size_t KEYSIZE) {
            ...
            for (size_t i = 0; i < size; ++i) {
                ...
                case 1: *(pointer +i) -= c; break;
                case 2: *(pointer +i) ^= c; break;
                }
                if (++p >= KEYSIZE) p = 0;
            }
            binary_pushint(self, crc); // embed crc32 to this binary
            return true;
        }
        
        bool binary_decrypt(binary_t *self, const std::string &key, size_t KEYSIZE) {
            if (self->data.empty()) return false; // if not new binary => fail
            ...
            // check crc32
            auto data = binary_open(self->head, size); // read binary except crc32
